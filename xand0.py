...
def minimax(max, min, new_move, isMax):
    if isMax:
        max.append(new_move)
        isMax = False
    else:
    ...
    if brd.is_max_win():
        ...
        for bm in blank_moves:
            value = minimax(max.copy(), min.copy(), bm, isMax)
            if isMax:
                if value==1:
                    return value
...
def best_move(maxMoves, minMoves):
    ...
    best = set()
    for bm in blank_moves:
        value = minimax(maxMoves.copy(), minMoves.copy(), bm, False)
        if value == -1:
            return bm
...
def game_loop():
    ...
    maxWin = winFont.render('Win', True, red)
    minWin = winFont.render('Lost', True, red)
